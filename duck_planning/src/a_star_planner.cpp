...
#include "duck_planning/a_star_planner.hpp"
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  // TEMPORARY: getGlobalFrameID() causes std::bad_alloc  
  // global_frame_ = costmap_ros->getGlobalFrameID();
  global_frame_ = "map";  // Hardcode for now
...
void AStarPlanner::activate()
{
  RCLCPP_INFO(
    ...
    name_.c_str());
  if (smooth_client_ && !smooth_client_->wait_for_action_server(std::chrono::seconds(3))) {
  ...
  std::function<bool()>)
{
  ...
  if(smooth_client_ && smooth_client_->action_server_is_ready()){
...
AStarPlanner::GraphNode AStarPlanner::worldToGrid(const geometry_msgs::msg::Pose & pose)
{
...
PLUGINLIB_EXPORT_CLASS(duck_planning::AStarPlanner, nav2_core::GlobalPlanner)