...
#include "duck_planning/dijkstra_planner.hpp"
...
{
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Configure: Step 1 - Locking parent node");
  node_ = parent.lock();
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Configure: Step 2 - Setting name and tf");
  name_ = name;
  tf_ = tf;
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Configure: Step 3 - Getting costmap");
  costmap_ = costmap_ros->getCostmap();
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Configure: Step 4 - Getting global frame");
  // Workaround: getGlobalFrameID() was causing std::bad_alloc - hardcode for now
  // global_frame_ = costmap_ros->getGlobalFrameID();
  global_frame_ = "map";

  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Configure: Step 5 - Creating smooth client");
  smooth_client_ = rclcpp_action::create_client<nav2_msgs::action::SmoothPath>(node_, "smooth_path");
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Configure: Complete!");
}
...
{
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Activate: Step 1 - Starting activation");
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Activate: Step 2 - Checking node_");
  if (node_) {
    RCLCPP_INFO(
      node_->get_logger(), "Activating plugin %s of type DijkstraPlanner",
      name_.c_str());
  }
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Activate: Step 3 - Checking smooth_client_");
  if (smooth_client_) {
    RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Activate: Step 4 - Waiting for action server");
    if (!smooth_client_->wait_for_action_server(std::chrono::seconds(3))) {
      RCLCPP_ERROR(rclcpp::get_logger("DijkstraPlanner"), "Action server not available after waiting");
    }
  }
  RCLCPP_INFO(rclcpp::get_logger("DijkstraPlanner"), "Activate: Complete!");
}
...
{
  RCLCPP_ERROR(rclcpp::get_logger("DijkstraPlanner"), "Calculated index out of bounds");
...
PLUGINLIB_EXPORT_CLASS(duck_planning::DijkstraPlanner, nav2_core::GlobalPlanner)