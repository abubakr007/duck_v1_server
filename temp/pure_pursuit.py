...
class PurePursuit(Node):
    ...
    def control_loop(self):
        ...
        # FIX: Update carrot frame_id to match the transformed path
        carrot_pose.header.frame_id = self.global_plan.header.frame_id
        carrot_pose.header.stamp = self.get_clock().now().to_msg()
    ...
    def get_carrot_pose(self, robot_pose: PoseStamped) -> PoseStamped:
        # Default to the goal if nothing is found
        ...
        for pose in self.global_plan.poses:
            ...
            # Find the first point at or beyond the lookahead distance
        ...
        return carrot_pose
    # def get_carrot_pose(self, robot_pose: PoseStamped) -> PoseStamped:
    #     carrot_pose = self.global_plan.poses[-1]  # Default to goal
    ...
    #         if distance >= self.look_ahead_distance:
    #             carrot_pose = pose
    #             break  # Found the first point at or beyond lookahead distance
    ...
    def transform_plan(self, frame):
        ...
        try:
            # Get transform FROM current plan frame TO target frame
        ...
        for pose in self.global_plan.poses:
            ...
            # Apply transform: result = transform * pose
            transformed_pose = concatenate_matrices(transform_matrix, pose_matrix)
            
            # Extract transformed pose
            quat = quaternion_from_matrix(transformed_pose)
            pose.pose.orientation.x = quat[0]
            pose.pose.orientation.y = quat[1]
            pose.pose.orientation.z = quat[2]
            pose.pose.orientation.w = quat[3]
            
            pose.pose.position.x = transformed_pose[0][3]
            pose.pose.position.y = transformed_pose[1][3]
            pose.pose.position.z = transformed_pose[2][3]
